...
public struct MoveGeneration
{
    ...
    private const ulong _FirstRank = 0xFF,
        _SecondRank = 0xFF00,
        _SeventhRank = 0xFF000000000000,
        _EighthRank = 0xFF00000000000000;
        ...
        for (int piece = 0; piece < Board.BitboardCount; piece++)
        {
            ...
            for (int iternator = 0; iternator < bits; iternator++)
            {
                ...
                switch (piece)
                {
                    ...
                    case 1:
                    case 7:
                        AttackedSquares[side] |= AttackTables.GetBishopAttacks(
                            srcSquare,
                            board.Occupancies[(int)Side.Both]
                        );
                        break;
                    case 2:
                    case 8:
                    ...
                    case 3:
                    case 9:
                        AttackedSquares[side] |= AttackTables.GetRookAttacks(
                            srcSquare,
                            board.Occupancies[(int)Side.Both]
                        );
                        break;
                    case 4:
                    case 10:
                        AttackedSquares[side] |= AttackTables.GetQueenAttacks(
                            srcSquare,
                            board.Occupancies[(int)Side.Both]
                        );
                        break;
    ...
    public static int[] InitMoves(Board board, Side side)
    {
                ...
                switch ((Piece)piece)
                {
                    case Piece.WPawn:
                        ...
                        else if (Helper.GetBit(board.Occupancies[(int)Side.Both], src + 8) == 0)
                        {
                            ...
                            if (
                                Helper.CheckBit(1ul << src, _SecondRank, src)
                                && Helper.GetBit(board.Occupancies[(int)Side.Both], src + 16) == 0
                            )
                            {
                                int move = Move.EncodeMove(
                                    src,
                                    src + 16,
                                    (Piece)piece,
                                    Piece.noPiece,
                        ...
                        attacks =
                            AttackTables.PawnAttacks[(int)side, src]
                            & board.Occupancies[(int)Helper.GetOpponent(side)];
                        ...
                        if (
                            ...
                            && (src + 7 == enpassant || src + 9 == enpassant)
                        )
                        {
                            int moves = Move.EncodeMove(
                                src,
                    ...
                    case Piece.BPawn:
                        ...
                        else if (Helper.GetBit(board.Occupancies[(int)Side.Both], src - 8) == 0)
                        {
                            ...
                            // Handles double pawn pushes
                            if (
                                Helper.CheckBit(1ul << src, _SeventhRank, src)
                                && Helper.GetBit(board.Occupancies[(int)Side.Both], src - 16) == 0
                            )
                            {
                                int move = Move.EncodeMove(
                    ...
                    case Piece.BBishop:
                    case Piece.WBishop:
                        attacks = AttackTables.GetBishopAttacks(
                            src,
                            board.Occupancies[(int)Side.Both]
                        );
                        possibleAttacks = Helper.CountBit(attacks);
                    ...
                    case Piece.WRook:
                    case Piece.BRook:
                        attacks = AttackTables.GetRookAttacks(
                            src,
                            board.Occupancies[(int)Side.Both]
                        );
                        possibleAttacks = Helper.CountBit(attacks);
                    ...
                    case Piece.BQueen:
                    case Piece.WQueen:
                        attacks = AttackTables.GetQueenAttacks(
                            src,
                            board.Occupancies[(int)Side.Both]
                        );
                        possibleAttacks = Helper.CountBit(attacks);
        ...
        if (side == Side.White)
        {
            if (((short)board.Rights & (short)CastlingRights.wk) > 0)
                moveList[moveIndex++] = CastlingMove(board, CastlingRights.wk);
            if (((short)board.Rights & (short)CastlingRights.wq) > 0)
                moveList[moveIndex++] = CastlingMove(board, CastlingRights.wq);
        }
        else if (side == Side.Black)
        {
            if (((short)board.Rights & (short)CastlingRights.bk) > 0)
                moveList[moveIndex++] = CastlingMove(board, CastlingRights.bk);
            if (((short)board.Rights & (short)CastlingRights.bq) > 0)
                moveList[moveIndex++] = CastlingMove(board, CastlingRights.bq);
        }
        return moveList;
    }
    ...
    private static int CastlingMove(Board board, CastlingRights right)
    {
        ...
        ulong castleMask,
            irrelevantSq,
            relevantRank = board.Occupancies[(int)Side.Both];
        switch (right)
        {
            case CastlingRights.wk:
                ...
                castleMask = 0x6;
                if (
                    (relevantRank & castleMask) == 0
                    && (AttackedSquares[(int)Side.White] & castleMask) == 0
                )
                    return Move.EncodeMove(
                        (int)Square.e1,
                        (int)Square.g1,
                        Piece.WKing,
                        Piece.noPiece,
            ...
            case CastlingRights.wq:
                ...
                if (
                    (relevantRank & castleMask) == 0
                    && (AttackedSquares[(int)Side.White] & (castleMask ^ irrelevantSq)) == 0
                )
                    return Move.EncodeMove(
                        (int)Square.e1,
                        (int)Square.c1,
            ...
            case CastlingRights.bk:
                ...
                if (
                    (relevantRank & castleMask) == 0
                    && (AttackedSquares[(int)Side.Black] & castleMask) == 0
                )
                    return Move.EncodeMove(
                        (int)Square.e8,
                        (int)Square.g8,
            ...
            case CastlingRights.bq:
                ...
                if (
                    (relevantRank & castleMask) == 0
                    && (AttackedSquares[(int)Side.Black] & (castleMask ^ irrelevantSq)) == 0
                )
                    return Move.EncodeMove(
                        (int)Square.e8,
                        (int)Square.c8,
                ...
                break;
            default:
                throw new System.Exception("Invalid Castling Flag!");
        }
        return 0;
    }
}
