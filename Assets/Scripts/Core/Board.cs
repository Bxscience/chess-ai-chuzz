...
public struct Board
{
    private const string _DefaultPosition =
    ...
    {
        int src = Move.GetSrcSquare(move),
            dest = Move.GetDestSquare(move);
        ...
        if (Move.IsCapture(move))
        {
            if (Move.IsEnpassant(move))
                Helper.PopBit(
                    ref Bitboards[(int)GetPieceFromSq((int)Enpassant - enpassantOffset)],
                    dest + (int)Enpassant
                );
            else
        ...
        else if (Move.IsCastle(move))
        {
            ...
            else if (PlayerTurn == Side.Black)
            {
                ...
                throw new Exception("How'd this happen");
            switch (dest)
            {
                case (int)Square.g1:
                ...
                case (int)Square.c8:
                    ...
                    break;
                default:
                    throw new Exception("Move Generation Error?");
            }
        }
        else if (Move.GetPromotedPiece(move) != Piece.noPiece)
        {
            if (PlayerTurn == Side.White) { }
            else { }
        }
    ...
    private Piece ValueSwitch(char c) =>
        c switch
        {
            'P' => Piece.WPawn,
            'B' => Piece.WBishop,
    ...
    private void ParseFENString()
    {
        ...
        Square result;
        if (Enum.TryParse(FENPartitions[3], out result))
            Enpassant = result;
        else if (FENPartitions[3][0] == '-')
            Enpassant = Square.noSq;
        else
            throw new Exception("Invalid FEN String");
    ...
    {
        Occupancies[(int)Side.White] =
            Bitboards[(int)Piece.WPawn]
            | Bitboards[(int)Piece.WBishop]
            | Bitboards[(int)Piece.WKnight]
            | Bitboards[(int)Piece.WRook]
            | Bitboards[(int)Piece.WQueen]
            | Bitboards[(int)Piece.WKing];
        Occupancies[(int)Side.Black] =
            Bitboards[(int)Piece.BPawn]
            | Bitboards[(int)Piece.BBishop]
            | Bitboards[(int)Piece.BKnight]
            | Bitboards[(int)Piece.BRook]
            | Bitboards[(int)Piece.BQueen]
            | Bitboards[(int)Piece.BKing];
