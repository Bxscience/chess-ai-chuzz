public struct AttackTables
{
    ...
    public readonly static ulong[,] PawnAttacks = new ulong[
        (int)Side.Both,
    ...
    public readonly static ulong[,] BishopAttacks = new ulong[
        Board.BoardSize * Board.BoardSize,
        512
    ];

    // Pregenerated Rook Attacks Table
    public readonly static ulong[,] RookAttacks = new ulong[
        Board.BoardSize * Board.BoardSize,
        4096
    ];
    public static readonly ulong[] BishopMasks = new ulong[Board.BoardSize * Board.BoardSize];
    public static readonly ulong[] RookMasks = new ulong[Board.BoardSize * Board.BoardSize];
    ...
    public static ulong GenerateBishopAttacks(int index, ulong blocker)
    {
        ...
        for (rank = targetRank + 1, file = targetFile + 1; rank <= 7 && file <= 7; rank++, file++)
        {
            ...
            attacks |= 1ul << square;
            if (Helper.CheckBit(1ul << square, blocker, square))
                break;
        }
    ...
    private static void InitSlidersAttacks(bool isBishop)
    {
        for (int square = 0; square < Board.BoardSize * Board.BoardSize; square++)
        {
            ...
            int relevantbits = isBishop
                ? Pregen.BishopRelevantBits[square]
                : Pregen.RookRelevantBits[square];
            ...
            for (int index = 0; index < occupancyIndicies; index++)
            {
                if (isBishop)
                {
                    ...
                    int magicIndex = (int)(
                        (occupancy * Pregen.BishopMagics[square]) >> (64 - relevantbits)
                    );
                ...
                else
                {
                    ulong occupancy = SetOccupancy(index, relevantbits, attackMask);
                    int magicIndex = (int)(
                        (occupancy * Pregen.RookMagics[square]) >> (64 - relevantbits)
                    );
    ...
    )
    {
        ...
        if (
            side == Side.White
            && (PawnAttacks[(int)Side.Black, square] & bitboards[(int)Piece.WPawn]) > 0
        )
            return true;
        // Checks attacks by black pawns
        if (
            side == Side.Black
            && (PawnAttacks[(int)Side.White, square] & bitboards[(int)Piece.BPawn]) > 0
        )
            return true;
        ...
        if (
            (
                ...
                & (
                    side == Side.White
                        ? bitboards[(int)Piece.WKnight]
                        : bitboards[(int)Piece.BKnight]
                )
            ) > 0
        )
            return true;
        ...
        if (
            (
                KingAttacks[square]
                & (side == Side.White ? bitboards[(int)Piece.WKing] : bitboards[(int)Piece.BKing])
            ) > 0
        )
            return true;
        ...
        if (
            (
                GetBishopAttacks(square, occupancies[(int)Side.Both])
                & (
                    side == Side.White
                        ? bitboards[(int)Piece.WBishop]
                        : bitboards[(int)Piece.BBishop]
                )
            ) > 0
        )
            return true;
        ...
        if (
            (
                GetRookAttacks(square, occupancies[(int)Side.Both])
                & (side == Side.White ? bitboards[(int)Piece.WRook] : bitboards[(int)Piece.BRook])
            ) > 0
        )
            return true;
