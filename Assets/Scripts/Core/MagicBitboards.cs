public struct MagicBitboards
{
    ...
    public static ulong FindMagicNumber(int square, int relevantbits, bool isBishop)
    {
        ...
        ulong attackMask = isBishop
            ? AttackTables.MaskBishopAttacks(square)
            : AttackTables.MaskRookAttacks(square);
        ...
        for (int index = 0; index < occupancyIndicies; index++)
        {
            ...
            attacks[index] = isBishop
                ? AttackTables.GenerateBishopAttacks(square, occupancies[index])
                : AttackTables.GenerateRookAttacks(square, occupancies[index]);
        }
        ...
        for (int count = 0; count < 0xFFFFFF; count++)
        {
            ...
            if (Helper.CountBit((attackMask * magicNumber) & 0xFF00000000000000) < 6)
                continue;
            int index;
            bool fail;
